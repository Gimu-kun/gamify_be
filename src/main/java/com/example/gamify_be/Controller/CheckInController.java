...
public class CheckInController {
    ...
    @PostMapping("/{id}")
    public ApiResponse<CheckIn> checkIn(@PathVariable String id, @RequestBody CheckInRequestDto req){
        return checkInService.checkIn(id,req.getCheckInDate(),req.getCheckInTime());
    }

    @GetMapping("/max_streak/{userId}")
    public ApiResponse<Integer> getMaxStreak(@PathVariable String userId){
        return checkInService.getMaxStreak(userId);
    }

    @GetMapping("/current_streak/{userId}")
    public ApiResponse<Integer> getCurrentStreak(@PathVariable String userId, @RequestParam LocalDate checkInDate){
