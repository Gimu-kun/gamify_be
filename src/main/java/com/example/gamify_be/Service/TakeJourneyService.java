...
public class TakeJourneyService {
    ...
    public ApiResponse<?> createTakeJourney(String userId, String journeyId){
        //Kiểm tra quan hệ đã tồn tại chưa
        if (isTakeJourneyExisted(userId,journeyId)){
            return ApiResponse.error("Quan hệ đã tồn tại", HttpStatus.NO_CONTENT);
        }

        //Kiểm tra người dùng có tồn tại không
        User user = userService.getUserById(userId);
        if (user == null){
            return ApiResponse.error("Không tìm thấy người dùng tương ứng", HttpStatus.NOT_FOUND);
        }
        ...
        try{
            ...
            return ApiResponse.success("Tham gia lộ trình thành công", null);
        }catch(Exception e){
            return ApiResponse.error("Lỗi trong quá trình khởi tạo quan hệ tk_jn",HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    //Hàm tìm kiếm danh sách người dùng đã tham gia lộ trình
    public ApiResponse<List<User>> getAllTakenUserByJourney(String journeyId) {
        ...
        List<User> users = takeJourneys.stream()
                ...
                .toList();
        return ApiResponse.success("Lấy danh sách người dùng tham gia thành công", users);
    }

    //Hàm tìm kiếm tất cả lộ trình người dùng chỉ định đã tham gia
    public ApiResponse<List<Journey>> getAllTakenJourneyByUser(String userId){
        ...
        List<Journey> journeys = takeJourneys.stream()
                ...
                .toList();
        return ApiResponse.success("Lấy thông tin danh sách lộ trình thành công",journeys);
    }
}
